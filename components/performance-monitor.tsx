...
import { BAND_BADGE, bandFor } from "@/lib/performance";
      ...
      <table className="w-full text-sm min-w-[720px]">
        <thead className="bg-paper-dark/50 text-xs text-ink-muted">
          <tr className="text-left">
            ...
            <th className="px-4 py-3 font-semibold text-right">Courses</th>
          ...
          {rows.map((r) => {
            ...
            return (
              <tr key={r.studentId} className="hover:bg-paper-dark/30">
                ...
                <td className="px-4 py-3 text-right tabular">{r.courses.length}</td>
...
// Course filter rendered as links so the page stays a server component.
export function CourseFilter({
  ...
  return (
    <div className="flex flex-wrap gap-2">
      <Link
        href={basePath}
        className={!active ? "badge badge-oxblood" : "badge badge-muted"}
      >
        All courses
      </Link>
      {courses.map((c) => (
        <Link
          key={c.id}
          href={`${basePath}?course=${c.id}`}
          title={c.title}
          className={active === c.id ? "badge badge-oxblood" : "badge badge-muted"}
        >
          {c.code}
        </Link>
      ))}
